...
type testClient struct {
	grpc.ServerStream
	c chan *v1alpha1.StreamDataResponse
}

func (tc *testClient) Send(m *v1alpha1.StreamDataResponse) error {
	tc.c <- m
	return nil
}

func (tc *testClient) SendAndClose(m *v1alpha1.StreamDataResponse) error {
	tc.c <- m
	tc.c <- nil
	return nil
}

func (tc *testClient) Recv() (*v1alpha1.StreamDataRequest, error) {

	r := <-tc.c
	if r == nil || r.StreamData == nil {
		return nil, io.EOF
	}
	return &v1alpha1.StreamDataRequest{StreamData: r.StreamData}, nil
...
func TestBiDirectStream(t *testing.T) {
	...
	tc := testClient{&ts, make(chan *v1alpha1.StreamDataResponse)}

	// go routine the BiDirectStream
	go s.BiDirectStream(&tc)

	// stream the testing data
	sd1 := v1alpha1.StreamDataResponse{
		StreamData: &v1alpha1.StreamData{
			Id:    1,
			Value: 1,
		},
	}
	sd2 := v1alpha1.StreamDataResponse{
		StreamData: &v1alpha1.StreamData{
			Id:    2,
			Value: 2,
		},
...
func TestClientStream(t *testing.T) {
	...
	for i := 1; i < 11; i++ {
		tc.Send(&v1alpha1.StreamDataResponse{
			StreamData: &v1alpha1.StreamData{
				Id:    int64(i),
				Value: float64(i),
			},
...
func TestServerStream(t *testing.T) {
	...
	go s.ServerStream(&v1alpha1.StreamDataRequest{
		StreamData: &v1alpha1.StreamData{
			Id:    1,
			Value: 1,
		},
